...
InputListener::InputListener(GLFWwindow* window) : window(window)
{
	...
	{
		instance = this;
		InputListener::getInstance()->window = window;
...
void InputListener::processInput()
{
	...
	{
		mouseAxisMoveCallback(InputListener::getInstance()->window, ImGui::GetMousePos().x, ImGui::GetMousePos().y);
...
{
	return glfwGetKey(InputListener::getInstance()->window, key);
}

int InputListener::pressedMouse(int key)
{
	int state = glfwGetMouseButton(InputListener::getInstance()->window, key);
	if (state == GLFW_PRESS)
	{
		glfwSetInputMode(InputListener::getInstance()->window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
		return GLFW_PRESS;
	}
	else if (state == GLFW_RELEASE)
	{
		glfwSetInputMode(InputListener::getInstance()->window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
		return GLFW_RELEASE;
	}
}
