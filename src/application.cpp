...
void shutdown()
{
	if (InputListener::pressedKey(GLFW_KEY_ESCAPE))
	{
		glfwTerminate();
	}
}
...
int main(void)
{
	...
	InputListener::getInstance()->addInputFunction(shutdown);
	...
	Shader S_light("resources/shaders/material.vert", "resources/shaders/light.frag");
	Shader S_lighting("resources/shaders/material.vert", "resources/shaders/lit.frag");
	...
	std::vector<Texture> txts{
		Texture("resources/assets/d_container.png", Texture::ImageType::PNG, Texture::TextureType::DIFFUSE, GL_REPEAT),
		Texture("resources/assets/s_container.png", Texture::ImageType::PNG, Texture::TextureType::SPECULAR, GL_REPEAT),
		Texture("resources/assets/matrix.jpg", Texture::ImageType::JPG, Texture::TextureType::EMISSIVE, GL_REPEAT)
	};
	...
	litObject->texture.push_back(txts.at(0).texture);
	litObject->texture.push_back(txts.at(1).texture);
	litObject->texture.push_back(txts.at(2).texture);
