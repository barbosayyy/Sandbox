...
void* FreelistAllocator::Allocate(size_t size, u8 alignment){
    ...
    Logger::Error("Memory Allocator: Failed to get Object pointer");
...
* WIP
*/
void* FreelistAllocator::Reallocate(void* pBlockPointer, size_t newSize){
    SB_NOT_IMPL;
    ...
    if(pBlockPointer != nullptr){
        ...
        void* pFullBlockStart = PMath::Subtract(pBlockPointer, pHeader->adjustment);
        size_t totalBlockSize = pHeader->size;
        if(newSize > totalBlockSize){

        }
    }
    return pBlockPointer;
}
...
PoolAllocator::PoolAllocator(size_t size, size_t blockSize) : Allocator(size), _blockSize(blockSize){
    assert(size >= sizeof(blockSize));
    _pBlocks->size = size;
    _pBlocks->pNextBlock = nullptr;
} 
...
void* PoolAllocator::Reallocate(void* p, size_t newSize){
